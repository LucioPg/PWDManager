...
mod diceware_tests {
    ...
    fn test_generate_diceware_default_config() {
        ...
        // No numeric-only words (numbers == 0)
        let words: Vec<&str> = pwd_str.split(|c: char| c.is_uppercase() && c != pwd_str.chars().next().unwrap())
            .filter(|s| !s.is_empty())
            .collect();
        for word in &words {
            assert!(!word.chars().all(|c| c.is_numeric()), "Word '{word}' should not be purely numeric");
        }
    }
