...
pub fn CloneToVaultDialog(
    ...
    /// The current vault ID (NOT used to filter -- clones can be same-vault)
    #[props(default = 0)]
    current_vault_id: i64,

    /// Callback when the user confirms the clone - passes the target vault_id
    ...
    // Fetch vault list (NOT filtering out current vault -- cloning within same vault is allowed)
    ...
    // Build combobox options -- all vaults are valid targets for cloning (no filtering)
